...
def eval_net(
    ...
    cv2.imwrite("conf.tif", (img.detach().cpu().numpy() * 255).astype(np.uint8)[0].transpose(1, 2, 0))
    cv2.imwrite("conf_gt.tif", (target.detach().cpu().numpy() * 255).astype(np.uint8)[0].transpose(1, 2, 0))
